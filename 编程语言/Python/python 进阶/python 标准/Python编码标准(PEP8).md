...
Python 3 不允许制表符和空格混合使用，因此，如果你使用的是Python3，解释器将会自动报错：
...
你可以自由选择哪个方式更清晰，但需要注意的是你必须在运算符的任何一侧使用相同数量的空格。下面的例子是不被允许的：
...
它可以通过命令行运行，就像 Linters 一样。假设你从名为 `code.py` 里面不符合PEP 8规范的代码开始：
